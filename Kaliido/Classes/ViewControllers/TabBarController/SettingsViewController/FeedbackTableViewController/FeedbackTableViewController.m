...
- (void)viewDidLoad {
    ...
    self.titles = @[NSLocalizedString(@"KL_STR_BUG", nil),
                    NSLocalizedString(@"KL_STR_IMPROVEMENT", nil),
                    NSLocalizedString(@"KL_STR_SUGGESTION", nil)];
...
- (IBAction)writeAnEmailTapped:(id)sender {
    ...
    [REMailComposeViewController present:^(REMailComposeViewController *mailVC) {
        ...
        NSString *recipient = @"bugs@kaliido.com";
        ...
        if (result == MFMailComposeResultSent) {
            ...
            [SVProgressHUD showSuccessWithStatus:NSLocalizedString(@"KL_STR_THANKS", nil)];
            [weakSelf.navigationController popViewControllerAnimated:YES];
            
        } else if (result == MFMailComposeResultFailed) {
            
            [[[UIAlertView alloc] initWithTitle:nil message:NSLocalizedString(@"KL_STR_MAIL_COMPOSER_ERROR_DESCRIPTION", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"KL_STR_CANCEL", nil) otherButtonTitles:nil] show];
        }
    }];
}
