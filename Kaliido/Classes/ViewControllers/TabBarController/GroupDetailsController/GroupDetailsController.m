...
@property (weak, nonatomic) IBOutlet KLImageView *groupAvatarView;
...
- (void)viewDidLoad {
    ...
    [[QMChatReceiver instance] chatAfterDidReceiveMessageWithTarget:self block:^(QBChatMessage *message) {
        ...
        if (message.cParamNotificationType == QMMessageNotificationTypeUpdateGroupDialog &&
            ...
            weakSelf.chatDialog = [[KLApi instance] chatDialogWithID:message.cParamDialogID];
...
- (IBAction)changeDialogName:(id)sender {
    ...
    [[KLApi instance] changeChatName:self.groupNameField.text forChatDialog:self.chatDialog completion:^(QBChatDialogResult *result) {
...
- (void)changeGroupAvatar:(id)sender {
    ...
    [QMImagePicker chooseSourceTypeInVC:self allowsEditing:NO result:^(UIImage *image) {
        [SVProgressHUD showWithMaskType:SVProgressHUDMaskTypeClear];
        [[KLApi instance] changeAvatar:[image imageByScaleAndCrop:CGSizeMake(900, 900)] forChatDialog:weakSelf.chatDialog completion:^(QBChatDialogResult *result) {
            if (result.success) {
...
{
    // check for friends:
    NSArray *friends = [[KLApi instance] contactsOnly];
    NSArray *usersIDs = [[KLApi instance] idsWithUsers:friends];
    NSArray *friendsIDsToAdd = [self filteredIDs:usersIDs forChatDialog:self.chatDialog];
    
    if ([friendsIDsToAdd count] == 0) {
        [[[UIAlertView alloc] initWithTitle:nil
                                    message:NSLocalizedString(@"KL_STR_CANT_ADD_NEW_FRIEND", nil)
                                   delegate:nil
                          cancelButtonTitle:NSLocalizedString(@"KL_STR_CANCEL", nil)
...
- (void)leaveGroupChat
{
    __weak typeof(self)weakSelf = self;
    [SVProgressHUD show];
    [[KLApi instance] leaveChatDialog:self.chatDialog completion:^(QBChatDialogResult *result) {
        [SVProgressHUD dismiss];
        if (result.success) {
